...
for sp_range in ranges_items.keys():
    ...
    playlist = sp.user_playlist_create(user=username, name=f'Personalized {sp_range} Playlist', public=False)
    sp.playlist_add_items(playlist_id=playlist['id'], items=ranges_items[sp_range])

    # Print the URL of the created playlist
    print(f"Playlist created successfully. You can access it at: {playlist['external_urls']['spotify']}")